...
class UserPropGen(User):
    ...
    def upload_spec(self):
        ...
        for m in self.modalities:
            ...
            # 获取 z_spec
            # 注意：这里我们只需要 z_spec
            _, z_spec, _ = self.ae_model.encode(x_m, m)       
            
            # 计算 z_spec 统计量
            # z_spec shape: [Batch, Time, Dim] -> Flatten -> [B*T, Dim]
            z_spec_flat = z_spec.reshape(-1, z_spec.shape[-1]) 
            
            spec_mean = z_spec_flat.mean(dim=0)
            spec_std = z_spec_flat.std(dim=0) + 1e-6 # 防止除0
    ...
    def train_ae_prop(self, global_share, global_prototypes, pre_w):
        ...
        for ep in range(self.local_epochs):
            ...
            while idx_end < seq_len:
                ...
                loss_dict_epoch['orth_loss'].append(orth_loss.item() if isinstance(orth_loss, torch.Tensor) else orth_loss)
                ...
                if server_logits_bank is not None:
                    ...
                    if has_teacher:
                        ...
                        with torch.no_grad():
                            ...
                            dynamic_alpha = torch.clamp(norm_entropy, 0.1, 0.8).mean().item()
